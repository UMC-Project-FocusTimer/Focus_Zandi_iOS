...
1. Timer, Follwer Tab에서 전송한 모든 데이터들을 이곳에서 확인 가능하다.(총 공부시간, 방해중단횟수, 팔로워 수, 잔디)
2. 오늘을 포함하여 공부한 날의 기록을 잔디로 확인 가능하며, 잔디색의 농도가 짙을 수록 공부시간이 높음을 의미한다.
3. Gear 버튼을 눌러 설정으로 들어간 후, '로그아웃' 버튼을 누르면 초기 로그인 화면으로 돌아간다.
